...
namespace LTW.Areas.Admin.Controllers
{
    ...
    {
        private readonly string connectionString = ConfigurationManager.ConnectionStrings["webdientuConnectionString"].ConnectionString;
        ...
        public ActionResult Restore(string fileName)
        {
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    // Restore database
                    string restoreQuery = $@"
                    RESTORE DATABASE [webdientu] 
                    FROM DISK = N'{filePath}'
                    WITH FILE = 1,
                    MOVE N'webdientu' TO N'C:\Program Files\Microsoft SQL Server\MSSQL16.MSSQLSERVER\MSSQL\DATA\webdientu.mdf',
                    MOVE N'webdientu_log' TO N'C:\Program Files\Microsoft SQL Server\MSSQL16.MSSQLSERVER\MSSQL\DATA\webdientu_log.ldf',
                    NOUNLOAD, REPLACE, STATS = 5;
                    
                    ALTER DATABASE webdientu SET MULTI_USER;";
